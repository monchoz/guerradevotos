...
def GetRegisteredUsers(maxResults=0, startsWith=''):
	usersList = []
	if startsWith:
		usersList = User.objects.filter(first_name__istartswith=startsWith, is_staff=0).values('first_name','social_auth__uid').order_by('first_name')
	else:
		usersList = User.objects.filter(is_staff=0).values('first_name','social_auth__uid').order_by('first_name')
	if maxResults > 0:
